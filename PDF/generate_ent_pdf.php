...
require('../conexion.php');
    ...
    $pdf->Cell(45, 10, 'Creacion', 1, 0, 'C', true);
    $pdf->Cell(15, 10, 'Items', 1, 0, 'C', true);
    $pdf->Cell(60, 10, 'Titulo', 1, 0, 'C', true);
    $pdf->Cell(90, 10, 'Cuerpo', 1, 0, 'C', true);
    $pdf->Cell(20, 10, 'Usuario', 1, 1, 'C', true);
    ...
    while ($row = $result->fetch_assoc()) {
        $pdf->SetX($startX);
        $pdf->Cell(10, 10, $row['id_entradas'], 1, 0, 'C');
        $pdf->Cell(45, 10, $row['fecha_creacion'], 1, 0, 'C');
        $pdf->Cell(15, 10, $row['items'], 1, 0, 'C');
        $pdf->Cell(60, 10, utf8_decode($row['titulo']), 1, 0, 'C');
        $pdf->Cell(90, 10, utf8_decode($row['body']), 1, 0, 'C');
        $pdf->Cell(20, 10, $row['id_user'], 1, 1, 'C');
    }
