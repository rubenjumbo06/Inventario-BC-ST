...
// Obtener el ID de la entrada desde el formulario POST
$id_entradas = isset($_POST['id_entradas']) && !empty($_POST['id_entradas']) ? $_POST['id_entradas'] : null;

if (!$id_entradas) {
    die("Error: No se proporcionó un ID de entrada válido.");
}

// Consulta SQL para obtener una sola entrada
...
$stmt->bind_param("i", $id_entradas); // "i" indica que es un entero
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    die("Error: No se encontró la entrada con el ID proporcionado.");
}
...
class PDF extends FPDF {
    function Header() {
        ...
        $this->SetY(20);
        $this->Cell(0, 20, 'Reporte de Entrada', 0, 1, 'C');
        $this->Ln(10);
    }
...
function parsearBody($body) {
    ...
    foreach ($partes as $parte) {
        $parte = trim($parte);
        if (strpos($parte, 'Herramientas:') === 0) {
...
// Mostrar los datos de la entrada seleccionada
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(0, 10, "Entrada #" . $row['id_entradas'], 0, 1, 'L');
$pdf->SetFont('Arial', '', 12);
...
header('Content-Disposition: attachment; filename="reporte_entrada_' . $row['id_entradas'] . '.pdf"');
header('Cache-Control: no-cache, no-store, must-revalidate');
header('Pragma: no-cache');
header('Expires: 0');

$pdf->Output('D', 'reporte_entrada_' . $row['id_entradas'] . '.pdf');
exit();
?>