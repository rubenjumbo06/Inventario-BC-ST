...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['id_tecnico'])) {
...
try {
    ...
    // Ocultar el técnico (cambiar id_status a 2 y actualizar fecha_eliminado)
    $sql = "UPDATE tbl_tecnico SET id_status = 2, fecha_eliminado = ? WHERE id_tecnico = ?";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        throw new Exception("Error preparando UPDATE tbl_tecnico: " . $conn->error);
    }
